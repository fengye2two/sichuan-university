	...
	for (int n = 0; n < imageCount_; ++n)
	{
		...
		for (int h = 0; h < cornerNumInHeight_; ++h)
		{
			for (int w = 0; w < cornerNumInWidth_; ++w)
			{
				...
				// lengthOfSquare_为打印出来的纸上，黑白正方形格子的实际宽度
				realPoint.y = h * lengthOfSquare_;
...
void SingleCalibrate::SingleCalibrateCamera(void)
{
	...
	calibrateCamera(realCorners3D_, imageCorners2D_, cv::Size(imageWidth_, imageHeight_), 
		            cameraMatrix_, distCoeffs_, rvecsMat_, tvecsMat_, 0);
}
