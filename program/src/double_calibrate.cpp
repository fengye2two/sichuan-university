...
void DoubleCalibrate::CalcTransformMap(void)
{
	...
	//R_left– 输出左侧相机的3x3矫正变换(旋转矩阵) .
	//R_right– 输出右侧相机的3x3矫正变换(旋转矩阵) .
	//P_left–在左侧相机的新的坐标系统(矫正过的)输出 3x4 的投影矩阵
	//P_right–在右侧相机的新的坐标系统(矫正过的)输出 3x4 的投影矩阵
	//Q–输出深度视差映射矩阵
...
void DoubleCalibrate::ShowRectified(std::string leftImage, std::string rightImage) {
	...
	// frame_left_rect拷贝至左半部分
	frame_left_rect.copyTo(combo(rectLeft));
	// frame_right_rect拷贝至右半部分
	frame_right_rect.copyTo(combo(rectRight));
